...
2. Email is sent to the guest with invitation link
3. Invitation expires after configured duration
4. Response includes the updated team with invitations array

**Response includes:**
- Updated team details
- All team members
- All pending invitations (including the new one)
...
3. You'll be redirected to the accept invitation page
4. The invitation will be automatically accepted

**Option 2: Via API**

```bash
curl -X POST http://localhost:8080/api/team-invitations/{teamId}/accept?invitationToken={token} \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -d '{
    "guest": "your-email@example.com",
    "invitationToken": "invitation_token_xyz"
  }'
```
...
```json
{
  ...
  "guest": "your-email@example.com",
  "expiryDate": "2026-01-28T10:00:00",
  "status": "ACCEPTED"
}
```
