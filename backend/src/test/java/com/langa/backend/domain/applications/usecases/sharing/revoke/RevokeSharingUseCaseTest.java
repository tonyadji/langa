...
class RevokeSharingUseCaseTest {
    ...
    @InjectMocks
    private RevokeSharingUseCase useCase;

    @Test
    void execute_shouldRevokeUserSharing() {
        ...
        ApplicationInfo info = useCase.execute(
                new RevokeSharingApplicationCommand("app-1", "owner@example.com", "guest@example.com", SharedWithProfile.USER));
    ...
    void execute_shouldThrow_whenCallerIsNotOwner() {
        ...
        assertThrows(ApplicationException.class, () -> useCase.execute(
                new RevokeSharingApplicationCommand("app-1", "intruder@example.com", "guest@example.com", SharedWithProfile.USER)));
    }

    @Test
    void execute_shouldThrow_whenNothingToRevoke() {
        Application app = Application.createNew("My App", "ACC-1", "owner@example.com");
        when(applicationRepository.findById("app-1")).thenReturn(Optional.of(app));
        when(userAccountService.getShareWithInfo("guest@example.com"))
                .thenReturn(new ShareWithInfo("ACC-GUEST", "guest@example.com"));
    ...
    @Test
    void revokeSharing_legacyMethod_shouldRevokeTeamSharing() {
        Application app = Application.createNew("My App", "ACC-1", "owner@example.com");
        app.shareWith("team-key", SharedWithProfile.TEAM);
        when(applicationRepository.findById("app-1")).thenReturn(Optional.of(app));
        when(teamService.getShareWithInfo("team-key")).thenReturn(new ShareWithInfo("team-key", "team@example.com"));
        when(applicationRepository.save(any(Application.class))).thenAnswer(inv -> inv.getArgument(0));

        useCase.revokeSharing("app-1", "owner@example.com", "team-key", SharedWithProfile.TEAM);

        assertFalse(app.alreadySharedWith("team-key"));
    }
